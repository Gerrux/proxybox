...
- Разбор Clash-YAML — подмножество: якоря (`&node` / `*node`), многострочные
  скаляры и списки блоком не читаются, а `ssr`, `snell` и `socks5` не наши
  протоколы вовсе; `tuic` наш, но из Clash-YAML в ссылку не перекладывается.
  Такой узел пропускается целиком, чтобы не завести профиль, который молча не
  соединяется; сколько узлов дошло, видно в журнале.
