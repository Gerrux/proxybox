...
- Расписание сверки подписок прибито гвоздями (шесть часов): настройки этому
  пока негде жить. Срок при этом считается от последней удачной сверки
  (`refreshed_at` в `state.json`), а не от старта службы, — подписка,
  просроченная за неделю простоя, сверяется на первом же тике потока, то есть
  через пять минут после запуска. Возраст списка виден в окне рядом с адресом.
