...
Пока такой прокси поднят, строка профиля помечена «браузер» (в CLI — строка
`браузер:` в `status`): иначе о втором sing-box в окне не сказано нигде. Метка
говорит про прокси, а не про окно — закрытие окна службе не видно.
...
Страну окно подписывает кодом («NL»), полное название — подсказкой: в строку
профиля «Нидерланды, Амстердам» не помещается. Флаг вместо кода не рисуется
намеренно — в Windows нет глифов флагов, пара региональных индикаторов
показалась бы теми же двумя буквами, только в квадратиках.
