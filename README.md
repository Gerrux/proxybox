...
Охват выбирают на шапке окна, у левого конца канала: «выбранные приложения»
либо «весь компьютер». Это не настройка — это то, кого касается инвариант, и
решают это глядя на состояние туннеля, а не через две панели от него. «Весь
компьютер» — отсутствие отбора, а не отбор целиком: список приложений в этом
режиме не применяется вовсе, и под туннель попадает трафик, за которым нет
процесса, — служба, драйвер, DNS.
...
- Белый список («выбранным — туннель, остальным сети нет») не проверен на живой
  Windows ни разу, и проверять там надо ровно одно: доходят ли пакеты выбранного
  приложения до нашего TUN по разрешающему правилу, пока политика брандмауэра
  запрещает весь исходящий. Во всех прежних охватах эта политика жила только те
  секунды, пока туннеля нет, и вопрос «пропустит ли она своих» не вставал.
  Не пропустит — выбранные приложения останутся без сети при поднятом туннеле,
  то есть охват окажется бесполезным, но не дырявым: fail-closed сохраняется.
- В белом списке имена невыбранных приложений всё равно разрешает сервер: секция
  `dns` на охват не смотрит, запрос уходит через туннель, и отказ приложение
  получает уже на самом соединении. Данные наружу не идут, но список доменов
  запертых приложений видит узел.
...
- Выборочность стоит процессора. Правило по `process_path` заставляет sing-box
  ...
  в конфиге для выборочного охвата нет, он и есть причина расхода. Как это
  снимается — `docs/wfp.md`: свой WFP-драйвер, разобранный до кода и упёршийся
  в подпись (EV-сертификат и Partner Center), а не в объём работы.
