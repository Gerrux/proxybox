...
- Обновление не ставится само: окно показывает релизы и открывает установщик в
  ...
  бы чем попало.
- «Вышло новее» окно решает сравнением строк: верхний релиз в списке отличается
  от нашей версии — значит новее. Semver не разбирается, поэтому патч к старой
  ветке, выпущенный после новой минорной, окажется сверху, и окно предложит
  уйти назад.
...
- Запущенные процессы попадают в список все, кроме начинки Windows
  ...
  ПО. Имя тут — имя файла, человеческого взять неоткуда. Снимок берётся одним
  заходом в буфер на 4096 записей: хвост сверх этого в список не попадёт.
