...
- Выборочность стоит процессора. Правило по `process_path` заставляет sing-box
  ...
  `final: direct` его сначала надо довести мимо этого правила. На Windows
  определение процесса — перебор таблицы соединений, то есть тем дороже, чем
  больше соединений открыто. Клиенты, которые гонят в туннель всё подряд и
  процессов не различают, этой цены не платят вовсе. Дешёвого обхода нет:
  сузить нагрузку можно, только отказавшись от выборочности.
...
- Подписка обновляется только руками. Расписание — это таймер в надзоре и
  ...
  Узел, вернувшийся из обновления под тем же именем, туннель переживает:
  приватный режим поднимается обратно сам.
...
- Кнопка «открыть вкладку Chromium через профиль»: запуск браузера с
  ...
  Порты для второго инстанса уже есть: `core-tunnel::free_port()`, им живёт
  прогон профилей — `Options::default()` прибит к 48292/48293, и на них второй
  инстанс не забиндится.
