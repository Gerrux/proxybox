...
Он показывает состояние формой, а не только цветом: круг цел — трафик идёт;
круг перерублен поперёк, как канал в шапке окна, — у выбранных приложений сети
нет (подключение, запертый доступ и неотвечающая служба — всё это одно и то же
с точки зрения приложения); кольцо — приватный режим выключен, продукт не
участвует. Рисуется значок в коде из тех же токенов, что и окно, — файлов .ico
под состояния нет.
