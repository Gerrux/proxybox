...
наши правила её только перекрывают.
...
Кнопка «Прогнать» в шапке списка профилей (в CLI — `proxybox test`)
Кнопка «Прогнать» в шапке списка профилей (в CLI — `privacy-gateway test`), а у
...
- Каталог `%ProgramData%\proxybox` создаётся с наследованными правами:
...
- Замок меняет политику брандмауэра по умолчанию и возвращает её в умолчание
  Windows (`blockinbound,allowoutbound`), а не в то, что стояло у пользователя:
  своя настройка исходящего до нас потеряется. В белом списке замок стоит всё
...
- В белом списке имена невыбранных приложений всё равно разрешает сервер. Щель
  для DNS открыта `svchost.exe` целиком (UDP/53 с адреса туннеля), а различить,
  какая служба внутри `svchost` попросила имя, нельзя ни в правиле, ни в WFP:
  `ALE_APP_ID` — это путь к `svchost.exe`, один на всех. Данные наружу не идут —
  само соединение приложения потом запрещено, — но список доменов запертых
  приложений узел видит.
