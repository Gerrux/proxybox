...
- Автообнаружение ищет в профиле того, кто нажал: `%USERPROFILE%` приходит от
  клиента (окно и CLI работают от имени человека, служба — от LocalSystem).
  Клиент постарше поля не пришлёт, и тогда служба по-прежнему перебирает все
  профили из `ProfileList` — на общей машине в списке окажутся чужие приложения.
  Правила брандмауэра всё равно ставятся по пути, то есть на всю машину: это про
  то, что человеку предлагают, а не про то, кого блокируют.
