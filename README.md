...
cargo run -p pg-cli -- browse --stop --profile myvpn   # погасить этот сеанс
...
## Окно браузера через профиль

Кнопка «Браузер» в строке профиля (в CLI — `privacy-gateway browse --profile
<имя>`) поднимает под этот профиль отдельный локальный прокси и открывает окно
браузера через него. Общий режим при этом не трогается: у инстанса свои порты,
свой каталог и **нет TUN**. Так и задумано — второй TUN с `auto_route` и
...
Сеансов бывает несколько разом, по одному на профиль: два окна в двух странах
работают одновременно и друг о друге не знают. Каталог у каждого свой
(`browser/<имя>` с хвостом-хешем от имени профиля) — `Tunnel::start` добивает
процесс из `singbox.pid`, и общий каталог означал бы, что каждый новый сеанс
гасит предыдущий. Порты тоже свои, `free_port()` на каждый.

Правил по `process_path` у этих инстансов нет: в туннель идёт тот, кто сам
указал прокси, — браузер запускается с `--proxy-server=socks5://127.0.0.1:<порт>`
и своим `--user-data-dir`. Каталог данных обязателен по двум причинам сразу: без
него Chromium передаёт аргументы уже запущенному окну и открывает обычную вкладку
мимо прокси, и он же делает сеансы независимыми — свои входы у каждого профиля.
Отсюда и главная неожиданность: окно открывается **чистым**, закладок и входов
из обычного Chrome в нём нет. Свои оно помнит до следующего раза, а удаление
профиля стирает их вместе с ним.

Ещё один аргумент — `--force-webrtc-ip-handling-policy=disable_non_proxied_udp`.
...
останется без сети, прокси откажет.

Пока сеанс поднят, строка профиля помечена «браузер» (в CLI — строка `браузер:`
в `status`): иначе о втором sing-box в окне не сказано нигде. Закрытие окна
службе не видно — она видит живой процесс, а тот переживает браузер легко,
— поэтому закрытия дожидается оболочка и сама шлёт `browse-stop`. Отдельной
кнопки «закрыть» нет за ненадобностью; в CLI есть `browse --stop --profile
<имя>`, там ждать некому.
...
Подделки user-agent и прочего отпечатка здесь нет и не планируется: `--user-agent`
меняет только строку, а `Sec-CH-UA`, `navigator.userAgentData`, часовой пояс,
шрифты и WebGL остаются настоящими. Chrome под Windows, представляющийся Safari
под macOS, — не маскировка, а отпечаток, которого больше нет ни у кого. Так
делают патченным Chromium, а не флагами.
...
- Сеанс браузера гаснет по закрытию окна, но ждёт закрытия окно приложения, а не
  служба. Закрыли Privacy Gateway раньше браузера — сеанс доживёт до остановки
  службы: без TUN, без правил, ничего не маршрутизируя. Числа сеансов никто не
