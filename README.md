...
который держат двое (один номер на двух локальных адресах), остаётся без имени:
назвать соединение чужим процессом хуже, чем не назвать никак.
...
- Кто держит соединение, служба узнаёт по локальному порту, и ключ у неё без
  локального адреса: два сокета на одном порту с разных адресов теряют имя оба
  (сторож — `a_clashing_port_has_no_owner`). Панель пишет им «без процесса»,
  потому что назвать соединение чужим процессом хуже, чем не назвать. Апгрейд —
  добавить в ключ адрес и сверять с `metadata.sourceIP` из Clash API.
