...
Rust-ядро в воркспейс-крейтах, служба над ним. На Windows 10/11 это ещё и
десктоп-оболочка на Tauri 2.x с фронтендом на Vite + React + TS + Tailwind —
интерфейс, служба и установщик говорят на шести языках. На Linux пока доступна
только служба и консоль (`proxybox`): своего окна, пакета и по-настоящему
белого списка там ещё нет — подробности в [docs/install.md](docs/install.md) и
...
Охватов два, и выбирают их на шапке окна (Windows) либо командой `proxybox
scope` (Linux). **Белый список** — сеть есть только у выбранных приложений и
только через туннель; на Linux эта грань пока не проведена, подробности в
[docs/limitations.md](docs/limitations.md). **Весь компьютер** — отбора нет
вовсе, под туннель попадает и трафик, за которым нет процесса: служба, драйвер,
DNS. Инвариант один и тот же, меняется только то, кого он касается.

Отбор живёт не в конфиге туннеля, а слоем ниже — в брандмауэре на Windows, в
nftables на Linux, — и происходит на `connect`, до всякого TUN. Конфиг sing-box
у обоих охватов побайтово один и тот же, маршрута мимо туннеля в нём нет вовсе
— поэтому переключение охвата и правка списка приложений туннель не
перезапускают: открытая SSH-сессия их переживает.
...
На Linux готового пакета пока нет: служба собирается и ставится вручную.
...
| [Установка](docs/install.md) | установщик и обновления на Windows, ручная установка службы на Linux, что помнит служба, чужой VPN рядом |
...
Windows — готовый продукт целиком: окно, служба, установщик. На Linux пока
доступна только служба и консоль, без окна, без пакета и без по-настоящему
белого списка ([docs/limitations.md](docs/limitations.md)). Дороже всего
сейчас — отчёт «поставил на живую машину, вот что вышло» с любой из платформ и
вычитка переводов: ни один из них, кроме английского, не читал носитель языка.
Остальное — в
