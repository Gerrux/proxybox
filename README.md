...
- Разбор Clash-YAML — подмножество. Метка якоря на узле (`- &node`) и списки
  блоком читаются, а ссылка на якорь (`*node`), слияние отображений (`<<:`) и
  многострочные скаляры — нет; `ssr`, `snell` и `socks5` не наши протоколы
  вовсе; `tuic` наш, но из Clash-YAML в ссылку не перекладывается — своего
  разбора `tuic://` у продукта нет. Такой узел пропускается целиком, чтобы не
