...
- Каталог приложений раскрывает `%USERPROFILE%`/`%LOCALAPPDATA%`/`%APPDATA%` и
  ищет в PATH из окружения службы, а служба работает под LocalSystem: это
  системный профиль внутри System32 и системный PATH, а не ваши. Записи,
  которые лежат только в домашнем каталоге пользователя и нигде не
  регистрируются (Claude Code, Codex), автообнаружение поэтому пропустит —
  добавлять их пока приходится руками, указав путь. Чинится передачей
  пользовательского окружения от клиента: и GUI, и CLI работают от обычного
  пользователя и знают верный `%USERPROFILE%`.
