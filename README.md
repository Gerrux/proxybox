...
- QR-кода нет: распознавание — это лишняя зависимость и снимок экрана ради
  ...
  поле есть.
- Третье состояние охвата — `none`: туннель поднят, но в него никто не идёт.
  Даёт «выключить, не выключая» для отладки. Заводится оно теперь не правилами
  в конфиге, а пустым списком пропусков.
