...
; Обновление не должно быть удалением. Tauri на установке поверх старой версии
; показывает страницу «уже установлено» и первым пунктом, отмеченным заранее,
; ставит «удалить перед установкой» — то есть зовёт деинсталлятор без /UPDATE.
; А тот считает себя настоящим удалением: открепляет ярлык от панели задач и
; «Пуска», стирает его, убирает автозапуск из HKCU\...\Run и удаляет службу. Новая
; версия ставит ярлык заново, но закреплённого у человека уже нет — и так на
; каждом обновлении.
;
; Поправить саму страницу из хуков нельзя (свой шаблон installer.nsi — это форк
; на тысячу строк, который разъедется с Tauri на первом же его обновлении), но
; переменная $UpdateMode у деинсталлятора глобальная, и хук PREUNINSTALL стоит
; раньше всех её проверок. Остаётся деинсталлятору узнать, что его позвал наш же
; установщик. Для этого установщик держит именованный мьютекс: Windows снимает
; его вместе с процессом, так что ни убирать метку, ни бояться, что она
; переживёт отменённую установку, не нужно. Удаление из «Параметров» идёт без
; живого установщика и остаётся полным. Сторож —
; `an_update_keeps_the_pinned_shortcuts`.
;
; MUI_CUSTOMFUNCTION_GUIINIT годится, потому что этот файл включён раньше
; страниц, а сама .onGUIInit отрабатывает раньше страницы «уже установлено».
; В тихом режиме (/S) её нет, но и страницы нет — удалять перед установкой там
; некому.
!define PB_SETUP_MUTEX "proxybox-setup"
!define MUI_CUSTOMFUNCTION_GUIINIT PbHoldSetupMutex

Function PbHoldSetupMutex
  ; Дескриптор не закрываем намеренно: мьютекс живёт ровно столько, сколько
  ; установщик.
  System::Call 'kernel32::CreateMutex(p 0, i 0, t "${PB_SETUP_MUTEX}") p .r0'
FunctionEnd
...
!macro NSIS_HOOK_PREUNINSTALL
  ; SYNCHRONIZE (0x00100000) — самое узкое право, с которым мьютекс открывается.
  System::Call 'kernel32::OpenMutex(i 0x00100000, i 0, t "${PB_SETUP_MUTEX}") p .r1'
  ${If} $1 P<> 0
    System::Call 'kernel32::CloseHandle(p r1)'
    ; Позвал установщик новой версии: ярлыки, закрепление и автозапуск
    ; оставляем ($UpdateMode — те самые проверки шаблона), а службу только
    ; гасим. Удалять её здесь и вовсе вредно: SCM держит удалённую службу
    ; помеченной MARKED_FOR_DELETE, и регистрация новой версии минутой позже
    ; упёрлась бы в это до перезагрузки. Правила брандмауэра снимает сама
    ; остановка.
    StrCpy $UpdateMode 1
    !insertmacro PB_SAY \
      "Остановка службы proxybox на время обновления..." \
      "Stopping the proxybox service for the duration of the update..." \
      "توقف سرویس proxybox در طول به‌روزرسانی..." \
      "更新期间停止 proxybox 服务..." \
      "Güncelleme süresince proxybox hizmeti durduruluyor..." \
      "Menghentikan layanan proxybox selama pembaruan..."
    DetailPrint $PB_MSG
    nsExec::ExecToLog 'net stop proxybox'
    Pop $0
  ${Else}
