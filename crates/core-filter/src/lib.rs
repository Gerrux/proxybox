...
//! секунд до двухсот миллисекунд не стоило ни того, ни другого.
//! Разобрано до кода — `docs/wfp.md`: там же, почему редирект на `mixed` из
//! первого замысла не работает и что делается вместо него.
