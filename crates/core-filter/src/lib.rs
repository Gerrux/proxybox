...
/// туннеля. Сам запрос уходит в TUN, перехватывается там (`hijack-dns`) и
/// разрешается сервером — наружу мимо туннеля не идёт ничего.
///
/// Различить, какая служба внутри `svchost` попросила имя, нельзя ни здесь, ни
/// в WFP: `ALE_APP_ID` — это путь к `svchost.exe`, один на всех. Значит узел
/// видит и имена, которые спрашивали запертые приложения, — записано в README.
...
pub fn set_fence(fence: Fence, tun_addr: &str, apps: &[String], browser: Option<&str>) -> io::Result<()> {
    sweep();
    if fence == Fence::Off {
        return Ok(());
    }
