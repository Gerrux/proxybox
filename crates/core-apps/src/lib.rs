...
/// Что у человека сейчас запущено — тот же обход, что и у автообнаружения, и
/// та же отсечка начинки Windows по каталогу.
///
/// Наружу он нужен службе ради ответа на `Request::Fenced`: заперто при
/// включённом приватном режиме всё, чего нет в белом списке, и назвать этот
/// список можно, только зная, что вообще работает. Второго обхода заводить
/// нельзя — разойдись он с автообнаружением отсечкой, и приложение, которое
/// показано запертым, невозможно было бы найти в списке, чтобы выбрать.
pub fn running() -> Vec<Found> {
    from_processes()
}
