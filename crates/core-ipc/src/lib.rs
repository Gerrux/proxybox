...
pub struct Conn {
    ...
    pub tunneled: bool,
    /// Выбранное приложение: оно есть в белом списке и получило пропуск.
    ///
    /// Считает это служба по той же причине, по какой считает утечку: путь у
    /// выбранного приложения бывает в двух формах — записанная и каноническая,
    /// — а в списке приложений живёт одна. Окно, сверяя путь процесса со своим
    /// списком, промахивалось бы ровно на той разнице, ради которой вторая
    /// форма и заведена, и предложило бы «добавить в белый список» то, что там
    /// уже есть. В охвате «весь компьютер» поле всегда `false`: белого списка
    /// там нет вовсе.
    #[serde(default)]
    pub picked: bool,
...
pub enum Request {
    ...
    ///
    /// `filter` — подстрока пути процесса или адреса назначения, пусто значит
    /// «всё». Отбор обязан считаться здесь, а не в окне: список режется сотней
    /// по громкости, и тихое соединение, которое как раз и искали, срезано ещё
    /// до того, как окну есть что фильтровать. Ответ на поиск получался бы
    /// пустым при живом соединении — тот же тихий промах, ради которого утечка
    /// поднимается наверх до обрезки. Сторож — `a_filter_is_applied_before_the_cut`.
    Connections {
        #[serde(default)]
        filter: String,
    },
    /// Кто сейчас заперт: запущенные приложения человека, которых нет в белом
    /// списке. Отбитого соединения среди них не значится ни одного, и это не
    /// упущение — следа от него не остаётся нигде: Windows отбивает его на
    /// `connect`, до sing-box оно не доходит, а сокет не успевает попасть в
    /// таблицу. Увидеть их можно было бы только аудитом WFP, то есть вечным
    /// журналом трафика на диске, которого продукт не ведёт.
    ///
    /// Поэтому вопрос перевёрнут: не «что отбито», а «кто заперт», и ответ на
    /// него выводится из уже известного, без единой новой записи. Это факт, а
    /// не догадка — при включённом приватном режиме невыбранное приложение
    /// заперто по определению, — но и не «оно стучалось»: молчащее приложение
    /// выглядит так же.
    ///
    /// В охвате «весь компьютер» список пуст всегда: делить там некого.
    Fenced,
...
pub enum Response {
    ...
    Proxy { port: u16 },
    /// Живые соединения. `total` — сколько их было всего, `matched` — сколько
    /// прошло отбор: в списке едут только самые говорливые, и без этих чисел
    /// обрезанный список читался бы как полный. Два числа, а не одно, потому
    /// что с непустым отбором «всего» — это вся машина, а не то, что искали.
    Connections {
        conns: Vec<Conn>,
        total: usize,
        #[serde(default)]
    ...
    /// Пути запущенных приложений, запертых прямо сейчас. Без имён и без
    /// иконок: имя окно достаёт из пути само, как достаёт его у соединения, а
    /// иконку спрашивает отдельной командой и только у тех строк, которые
    /// показывает.
    Fenced { apps: Vec<String> },
...
mod tests {
    ...
    fn roundtrip() {
        ...
        let resps = [
            ...
            Response::Fenced { apps: vec![r"C:\Program Files\qBittorrent\qbittorrent.exe".into()] },
            Response::Imported {
                added: 12,
                kept: 3,
                gone: 1,
    ...
    /// Липкий заголовок группы подписки обязан лежать на самой кромке
    /// прокрутки. Липкое считается не от кромки, а от поля прокрутки за
    /// вычетом её собственного отступа: с `p-3.5` на `.scroll` заголовок
    /// вставал на 14 px ниже (замерено на живой странице: кромка 420.8,
    /// заголовок 434.8), и в полоске над ним были видны строки, уезжающие под
    /// него — то есть липкое не закрывало собой ровно то, ради чего оно и
    /// липкое.
    ///
    /// Поэтому отступа у плиты по умолчанию нет вовсе: панель с обычным
    /// содержимым просит его сама (`pad="p-3.5"`), а список профилей — не
    /// просит и раздаёт отступы внутри себя. Вложенный блок с отступом решил
    /// бы то же самое, но молча возвращал бы ту же полоску всякому, кто
    /// переставит отступ обратно на прокрутку. И `top` у `.sub-head` — ноль,
    /// не компенсация отступа числом: такое число разъехалось бы с отступом
    /// молча. Компилятора у фронтенда нет — сверяем текстом.
    #[test]
    fn the_sticky_head_sits_on_the_scroll_edge() {
        let ui = include_str!("../../../ui/app-shell/src/ui.tsx");
        let panel = ui
            .split("export function Panel(")
            .nth(1)
            // Не по `\n}`: он попадает на закрывающую скобку пропсов, то есть
            // на строку выше самой разметки.
            .and_then(|s| s.split("\nexport function").next())
            .expect("плита живёт в Panel");
        assert!(panel.contains("pad = \"\""), "плита снова раздаёт отступ по умолчанию");
        let scroll = panel
            .lines()
            .find(|l| l.contains("className={`scroll"))
            .expect("прокрутка плиты размечена классом scroll");
        assert!(scroll.contains("${pad}"), "прокрутка не берёт отступ у панели");
        let padded = scroll
            .split_whitespace()
            .any(|c| c.starts_with("p-") || c.starts_with("pt-") || c.starts_with("py-"));
        assert!(!padded, "отступ снова стоит на самой прокрутке: липкий заголовок встанет ниже кромки");

        // Список профилей — единственный, кто отступа не просит: липкое живёт
        // только у него.
        let profiles = include_str!("../../../ui/app-shell/src/Profiles.tsx");
        let opening = profiles
            .split("<Panel")
            .nth(1)
            .and_then(|s| s.split(">").next())
            .expect("список живёт в плите");
        assert!(!opening.contains("pad="), "список профилей просит отступ у прокрутки");
        ...
        let head = css
            ...
            .nth(1)
            .and_then(|s| s.split('}').next())
            .expect("заголовок группы живёт в .sub-head");
        assert!(head.contains("top: 0;"), "заголовок липнет не к кромке, а с компенсацией числом");
    }
