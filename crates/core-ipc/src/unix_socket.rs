...
fn chgrp(target: &Path) {
    // Группу заводит пакет, а он приезжает подпроектом 3. Пока её нет, ни
    // каталога, ни сокета в системе для неё нет вовсе — то есть службой
    // управляет только root, и это честнее, чем открыть их кому-то ещё.
    // Отказ поэтому не отказ: `chgrp` на несуществующую группу означает
    // «пакета ещё не было».
