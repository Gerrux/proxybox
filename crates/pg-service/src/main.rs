...
/// Сколько промахов пробы подряд означают «туннеля нет». Один не означает
/// ничего: проба — это новое TCP-соединение к серверу каждые три секунды, то
/// есть самая хрупкая связь из всех, что у нас есть, и под потерями пакетов
/// она отваливается первой, пока живые соединения через тот же туннель
/// продолжают идти. Запертый на одном промахе брандмауэр рвал именно их —
/// SSH, загрузки, звонки, — и это не окно утечки, а разрыв на ровном месте.
///
/// Утечки в терпении нет, и по той же причине, по которой её нет в трёх
/// лишних секундах `PROBE_TIMEOUT`: маршрута мимо туннеля в конфиге не
/// осталось (`final: proxy`, тега `direct` нет вовсе), а смерть процесса ловит
/// не проба, а `DEATH_EVERY` — она замок ставит по-прежнему мгновенно.
/// Терпение касается только живого процесса, который не пропускает наружу.
///
/// Платим за это тем, что уже мёртвый туннель показан живым до полуминуты:
/// круг промаха — это `PROBE_EVERY` плюс дозвон sing-box. Разменяно
/// сознательно: поздний замок стоит приложениям отказов, которые они переживут
/// сами, а ложный — оборванного соединения, которое человек восстанавливает
/// руками. Сторож — `a_single_missed_probe_does_not_cut_the_selected_apps`.
...
struct Service {
    ...
    /// Промахов пробы подряд. Обнуляется удачной пробой, а поскольку `Up`
    /// ставится только там же, в подтверждённый туннель счётчик из прошлой
    /// жизни не переезжает.
    misses: u32,
...
/// Предлагается ли этот охват на текущей платформе.
///
/// На Linux белый список отбирать нечем: матчера по пути исполняемого файла в
/// netfilter нет, а cgroup приезжает следующим подпроектом. Правило `oifname`
/// на TUN выдаёт туннель всем процессам без разбора, поэтому включённый белый
/// список неотличим от «весь компьютер» — но человек, отметив приложения,
/// решил бы, что в туннель идут только они. Инвариант при этом цел в обоих
/// охватах: мимо туннеля не уходит никто, — так что отказываем не ради
/// приватности, а чтобы не обещать несуществующего отбора.
///
/// `Scope::None` не запрещаем: он ничего не обещает, пропусков не выдаёт
/// никому и ставится только из CLI. Сторож —
/// `the_whitelist_is_refused_aloud_on_linux`.
fn scope_offered(scope: Scope, linux: bool) -> bool {
    !(linux && scope == Scope::Whitelist)
}
...
fn handle(svc: &Mutex<Service>, req: Request) -> Response {
    ...
    match req {
        ...
        Request::SetScope { scope } => {
            // На Linux белый список отбирать нечем: в netfilter нет матчера по
            // пути исполняемого файла, а cgroup приезжает следующим
            // подпроектом. Правило `oifname` на TUN выдаёт туннель всем
            // процессам без разбора, поэтому включённый белый список был бы
            // неотличим от «весь компьютер» — но человек, отметив приложения,
            // решил бы, что в туннель идут только они. Отказ вслух дешевле
            // молчаливого обмана; инвариант при этом цел в обоих охватах.
            //
            // Диагностический `Scope::None` не запрещаем: он ничего не
            // обещает, пропусков не выдаёт никому и ставится только из CLI.
            // Сторож — `the_whitelist_is_refused_aloud_on_linux`.
            if !scope_offered(scope, cfg!(target_os = "linux")) {
                return Response::Error {
                    message: t("белый список на Linux пока не работает: отбирать по приложению нечем до cgroup, и охват «весь компьютер» — единственный честный"),
                };
            }
...
/// Пора ли запирать после промаха пробы. Терпим только промахи подтверждённого
/// туннеля: там есть что рвать — живые соединения и выданные пропуска. Пока
/// туннель не подтверждён, выбранные приложения и так заперты (`start()` запер
/// их до запуска процесса), терять нечего, и откладывать нечего тоже — молчать
/// в «Подключение» лишние полминуты значит прятать от человека мёртвый узел.
fn locks_after_miss(state: TunnelState, misses: u32) -> bool {
    state != TunnelState::Up || misses >= PROBE_MISSES
}
...
fn supervise(svc: &Arc<Mutex<Service>>) {
    ...
    loop {
        ...
        // Код SOCKS человеку не говорит ничего: «не пропустил соединение (код
        // 1)» — это и не дошедший до сервера SYN, и отказ в рукопожатии, и
        // живой узел, который не пропускает. Настоящую строку пишет sing-box в
        // свой лог, и раньше её приходилось сводить с лентой вручную, складывая
        // секунды аптайма со временем запуска. Тянем до `lock`: файл местный,
        // но это всё же диск, а под общим замком стоит окно, `supervise` и
        // наблюдатель за смертью. Платим этим чтением только пока туннель
        // лежит — на удачной пробе его нет вовсе.
        //
        // Строка помечена «последнее», и это не вежливость: свежесть её ничем
        // не подтверждена. Пока проба переживает дозвон, ответивший sing-box
        // успевает и записать — но если он завис и мы вышли по своему сроку,
        // последним в логе останется отказ получасовой давности. Названная
        // последней, она остаётся подсказкой; выданная за причину, отправила бы
        // искать не то.
        let result = result.map_err(|e| match core_tunnel::last_failure(&dir()) {
            Some(why) => std::io::Error::other(format!("{e}; {}", tf!("последнее от sing-box: {}", why))),
            None => e,
        });
        ...
        match result {
            ...
            Err(e) => {
                s.misses += 1;
                if locks_after_miss(s.status.tunnel, s.misses) {
...
mod tests {
    ...
    /// Белый список на Linux отказывает вслух, а не делает вид, что работает.
    ///
    /// Отбирать там нечем до cgroup, и включённый белый список неотличим от
    /// «весь компьютер» — но человек, отметивший приложения, решил бы, что в
    /// туннель идут только они. Молчаливое согласие тут хуже отказа: инвариант
    /// цел в обоих охватах, а вот обещание отбора было бы ложным.
    #[test]
    fn the_whitelist_is_refused_aloud_on_linux() {
        assert!(!scope_offered(Scope::Whitelist, true), "белый список на Linux обещает отбор, которого нет");
        assert!(scope_offered(Scope::Whitelist, false), "на Windows белый список работает и отказывать не в чем");
        assert!(scope_offered(Scope::All, true), "«весь компьютер» на Linux работает полностью");
        // Диагностический охват ничего не обещает и пропусков не выдаёт никому:
        // запрещать его незачем, а из CLI он нужен.
        assert!(scope_offered(Scope::None, true), "диагностический охват не обещает отбора и потому не запрещён");
    }
    ...
    /// Одиночный промах пробы — это не «туннеля нет». Проба открывает новое
    /// соединение к серверу каждые три секунды и под потерями пакетов
    /// отваливается первой, пока живые соединения через тот же туннель идут как
    /// шли. Замок на первом промахе рвал именно их — SSH, загрузки, звонки, — и
    /// не закрывал при этом ничего: маршрута мимо туннеля в конфиге нет вовсе
    /// (сторож `nothing_can_go_direct`), а смерть процесса ловит `DEATH_EVERY` и
    /// запирает по-прежнему мгновенно.
    #[test]
    fn a_single_missed_probe_does_not_cut_the_selected_apps() {
        assert!(PROBE_MISSES >= 2, "терпение меньше двух промахов — это отсутствие терпения");
        for miss in 1..PROBE_MISSES {
            assert!(!locks_after_miss(TunnelState::Up, miss), "промах {miss} из {PROBE_MISSES} запирает раньше срока");
        }
        assert!(locks_after_miss(TunnelState::Up, PROBE_MISSES), "промахи подряд обязаны запирать");

        // Неподтверждённый туннель терпению не подлежит: выбранные приложения
        // там и так заперты (`start()` запер их до запуска процесса), рвать
        // нечего — а лишние полминуты в «Подключении» прячут от человека
        // мёртвый узел.
        for state in [TunnelState::Down, TunnelState::Connecting] {
            assert!(locks_after_miss(state, 1), "{state:?}: неподтверждённый туннель обязан запираться сразу");
        }

        // Терпение живёт только у пробы. Наблюдение за смертью процесса
        // запирает без счёта: заглянь счётчик и туда, окно утечки после падения
        // sing-box стало бы в `PROBE_MISSES` раз шире того, ради чего заведён
        // `DEATH_EVERY`.
        let src = include_str!("main.rs");
        let body = src
            .split("fn watch_for_death(")
            ...
            .and_then(|s| s.split("\nfn ").next())
            .expect("наблюдатель за смертью на месте");
        assert!(!body.contains("misses"), "счёт промахов не имеет права попадать в наблюдение за смертью: {body}");
    }
