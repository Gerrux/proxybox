...
use core_ipc::{
    dir_name, t, App, BrowserProfile, Conn, Endpoint, Listener, LogLine, Probe, ProfileInfo, Request,
    Response, Scope, Settings, Status, Stream, Subscription, TestRun, Tunnel as TunnelState, ADDR,
};
...
impl Service {
    ...
    fn load() -> Self {
        ...
        let mut me = Self {
            status: Status {
                ...
                subscriptions: subscriptions_of(&saved.subscriptions, &saved.sub_names, &saved.profiles),
                probes: saved.probes,
    ...
    fn save(&mut self) {
        ...
        self.status.subscriptions = subscriptions_of(&self.subscriptions, &self.sub_names, &self.profiles);
...
mod tests {
    ...
    fn a_subscription_does_not_carry_names_of_deleted_nodes() {
        ...
        let out = subscriptions_of(&subs, &names, &profiles);
