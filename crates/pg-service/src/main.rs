...
mod tests {
    ...
    fn the_state_directory_is_locked_to_system_and_admins() {
        ...
        // На Windows `secure_dir` зовёт настоящий icacls с `/setowner`, а это
        // элевация, которой у обычного тестового процесса нет. На unix
        // `icacls` — пустышка, а `chmod` до 0o700 не требует прав вовсе,
        // поэтому там зовём саму функцию и проверяем её настоящий эффект, а не
        // только контракт команды.
        #[cfg(unix)]
