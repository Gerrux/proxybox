        ...
        Some("add-browser") => match (flag(args, "--name"), flag(args, "--node")) {
            (Some(name), Some(node)) => Ok(Request::SetBrowserProfile {
                profile: core_ipc::BrowserProfile {
                    ...
                    icon: String::new(),
                    // Метки и одноразовость — дело списка в окне: консоль
                    // заводит профиль, отбирать в ней нечего.
                    tags: Vec::new(),
                    ephemeral: false,
                    engine: core_ipc::Engine::Chromium,
                    // Консоль профиль заводит, но страну узла не знает — пояс
                    // выбирают в окне.
                    timezone: String::new(),
                    // Без отдельного флага CLI выбирает рабочий с обычными
                    // сайтами режим, как и форма нового профиля.
                    compatibility: true,
                },
            }),
