...
pub fn build_config(node: &Value, opts: &Options) -> Value {
    ...
    if opts.tun {
        ...
        // просто дешевеет каждая его итерация. `drop` заставляет приложение
        // ждать собственного таймаута, и частота повторов падает сама. Заодно
        // это честная эмуляция: на живой сети по несуществующему адресу молчат,
        // а не отвечают отказом.
        //
        // ponytail: отбиваем весь трафик на шлюз, а не только 1900 и 5351.
        // Разбирать порты незачем — по этому адресу нет ничего вообще, — но
        // если однажды появится, начинать надо отсюда.
        rules.insert(
            0,
            json!({ "ip_cidr": [format!("{TUN_GATEWAY}/32")], "action": "reject", "method": "drop" }),
        );
    }
...
mod tests {
    ...
    fn the_tun_gateway_is_rejected_first() {
        ...
        assert_eq!(first["ip_cidr"][0], format!("{TUN_GATEWAY}/32"));
        ...
        // Матчер по процессу правило не заводит — иначе лекарство стоило бы
        // дороже болезни.
