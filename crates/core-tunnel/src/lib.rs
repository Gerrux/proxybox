...
/// то, за что платила вся машина, — размен записан в `docs/limitations.md`.
///
...
?publickey=QUJDREVGR0hJSktMTU5PUFFSU1RVVldYWVphYmNk&address=10.0.0.2/32",
    ...
    /// Имя TUN на Linux зашито в `core-filter` строкой: у крейта нет
    /// зависимостей, и тянуть туда `core-tunnel` ради одной константы дороже,
    /// чем сверить текстом. Цена расхождения несимметрична — в белом списке
    /// `oifname` перестанет совпадать с настоящим адаптером, и вся машина,
    /// кроме root, останется без сети навсегда, молча, потому что таблица
    /// встанет успешно.
    #[test]
