...
/// компилятор: сама функция и `paint_icon` ниже под `cfg(windows)`, и job
/// `linux` в CI, который теперь собирает `src-tauri` и под Linux, этой ветки
/// не касается вовсе — она по-прежнему живёт только на Windows.
///
