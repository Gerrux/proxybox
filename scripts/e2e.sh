...
# TUN не поднимаем даже под root, и это не осторожность, а устройство стенда:
# VLESS-сервер здесь живёт на той же машине и на петле. Под живым TUN
# sing-box с `auto_detect_interface` привязывает исходящее к физическому
# интерфейсу, и петлевой адрес становится недостижим — `dial tcp
# 127.0.0.1:10443: cannot assign requested address`. Сперва дозванивается,
# после переопределения маршрутов перестаёт: проба успевает пройти, а
# следующие соединения уже нет.
#
# Замку это не мешает вовсе — он на TUN не смотрит. Таблица nftables ставится
# из `guard`, `policy drop` запирает всё, кроме петли, uid службы и
# интерфейса туннеля, и «выбранные приложения без сети при неподтверждённом
# туннеле» проверяется полностью. Непроверенным остаётся сам TUN: для него
# нужен сервер на другой машине, а его у CI нет. Записано в фазу 0.
export PG_TUN=0
...
step() { printf '\n== %s\n' "$1"; }
# Причину отказа туннеля пишет не проба, а сам sing-box, и лежит она в его
# журнале. В CI до неё иначе не добраться вовсе: каталог состояния уезжает
# вместе с $WORK на выходе, и разбирать провал приходится вслепую, круг за
# кругом. Хвост, а не файл целиком: на болтливом узле он растёт мегабайтами.
fail() {
  echo "ПРОВАЛ: $1" >&2
  if [ -f "$XDG_CONFIG_HOME/proxybox/singbox.log" ]; then
    echo "--- хвост singbox.log ---" >&2
    tail -30 "$XDG_CONFIG_HOME/proxybox/singbox.log" >&2
  fi
  exit 1
}
...
if [ "$FULL" = "1" ]; then
  ...
  # Цель — адресом, а не именем, и это не мелочь. Предмет этой проверки —
  # замок, а не разрешение имён; взяв имя, мы поставили бы её в зависимость от
  # того, как под живым TUN работает DNS, — а это отдельный открытый вопрос
  # (`systemd-resolved` слушает на `lo`, и правило `hijack-dns` на `tun-in`
  # его не видит; см. фазу 0 в спеке). Первая же редакция с `github.com`
  # именно на этом и упала, ничего не сказав про замок. Голый http, а не
  # https, по той же причине: TLS тут тоже не предмет.
  # 15, а не 5: round-trip до внешнего адреса под нагруженным CI-раннером в
  ...
  outsider() { setpriv --reuid=nobody --regid=nogroup --clear-groups \
      curl -s --max-time 15 -o /dev/null http://1.1.1.1; }
