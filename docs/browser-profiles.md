...
## Совместимость с проверками сайтов

Флажок **«Совместимость с проверками сайтов»** включён у новых и существующих
профилей по умолчанию. В этом режиме сайт видит цельное настоящее окружение
установленного браузера: Chromium запускается без DevTools-канала и игнорирует
сохранённую подмену UA и часового пояса, а Firefox явно выключает
`privacy.resistFingerprinting`. Это режим для Cloudflare и других проверок,
которые считают DevTools-подмену или RFP признаком управляемого браузера.

Изоляция профиля от этого не пропадает: каталог с куками и входами остаётся
отдельным, трафик по-прежнему идёт через SOCKS своего узла, WebRTC не получает
прямого UDP-пути, а Firefox не откатывается на прямое соединение и разрешает
имена через прокси. Цена совместимости — настоящий системный часовой пояс,
платформа и отпечаток машины видны сайту. Снятый флажок возвращает расширенную
личность, описанную ниже.

## Firefox: защита от отпечатка

Chromium умеет подменить строку UA с подсказками, язык и часовой пояс (об этом
ниже), а всё остальное в отпечатке у него настоящее. Firefox с выключенной
совместимостью умеет другое — `privacy.resistFingerprinting`
...
При выключенной совместимости всё это окно получает через DevTools-канал
(`core_apps::cdp`), который оболочка держит, пока окно открыто:
